...
from glue_eval.xnli_eval import XNLIEval
from glue_eval.pawsx_eval import PAWSXEval
from glue_eval.mlqa_eval import MLQAXEval
from glue_eval.wikiann_eval import WIKIANNEval
...
class GLUEEval():
    def __init__(self, model, tokenizer, number_of_tests = None, sst_number_of_few_shots = 0, mrpc_number_of_few_shots = 0, cola_number_of_few_shots = 0, rte_number_of_few_shots = 0, mmlu_number_of_few_shots = 0, sentiment_analysis_number_of_few_shots = 0, nli_number_of_few_shots = 0, dialogue_number_of_few_shots = 0):
        ...
        for lang_s in ['en', 'de', 'nl', 'es', 'fr', 'zh']:
            self.xnli_eval = XNLIEval(model, tokenizer, number_of_tests = number_of_tests, number_of_few_shots = 2, lang_s=lang_s)
            self.xnli_eval_dict[lang_s] = self.xnli_eval

        self.pawsx_eval_dict = {}
        for lang_s in ['en', 'de', 'es', 'fr']:
            self.pawsx_eval = PAWSXEval(model, tokenizer, number_of_tests = number_of_tests, number_of_few_shots = 2, lang_s=lang_s)
            self.pawsx_eval_dict[lang_s] = self.pawsx_eval

        self.wikiann_eval_dict = {}
        for lang_s in ['en', 'de', 'nl', 'es', 'fr', 'zh']:
            self.wikiann_eval = WIKIANNEval(model, tokenizer, number_of_tests = number_of_tests, number_of_few_shots = 3, lang_s=lang_s)
            self.wikiann_eval_dict[lang_s] = self.wikiann_eval

        self.mlqa_eval_dict = {}
        for lang_s in ['en', 'de', 'es', 'zh']:
            self.mlqa_eval = MLQAXEval(model, tokenizer, number_of_tests = number_of_tests, number_of_few_shots = 2, lang_s=lang_s)
            self.mlqa_eval_dict[lang_s] = self.mlqa_eval
    ...
    def evaluate(self, glue_results, record_path, perplexity_flag = False, sst_flag = False, mmlu_flag = False, mrpc_flag = False, cola_flag = False, rte_flag = False, nli_flag = False, sentiment_analysis_flag = False, dialogue_flag = False, xnli_flag = False, pawsx_flag = False, mlqa_flag=False, wikiann_flag=False, gen_len = 5):
        if perplexity_flag:
            raw_ds = load_dataset(
                        "wikitext",
        ...
        if mlqa_flag:
            for lang_s in ['en', 'de', 'es', 'zh']:
                # ipdb.set_trace()
                result_dict, generations = self.mlqa_eval_dict[lang_s].evaluate(print_logs='True', lang_s=lang_s)
                glue_results['mlqa_{}'.format(lang_s)] = result_dict
                self._save_generations(record_path, generations, 'mlqa_{}'.format(lang_s))

        if wikiann_flag:
            for lang_s in ['en', 'de', 'nl', 'es', 'fr', 'zh']:
                # ipdb.set_trace()
                result_dict, generations = self.wikiann_eval_dict[lang_s].evaluate(print_logs='True', lang_s=lang_s)
                glue_results['wikiann_{}'.format(lang_s)] = result_dict
                self._save_generations(record_path, generations, 'wikiann_{}'.format(lang_s))

        if pawsx_flag:
            for lang_s in ['en', 'de', 'es', 'fr']:
                # ipdb.set_trace()
                result_dict, generations = self.pawsx_eval_dict[lang_s].evaluate(print_logs='True', lang_s=lang_s)
                glue_results['pawsx_{}'.format(lang_s)] = result_dict
                self._save_generations(record_path, generations, 'pawsx_{}'.format(lang_s))

        if xnli_flag:
            for lang_s in ['en', 'de', 'nl', 'es', 'fr', 'zh']:
                # ipdb.set_trace()
                result_dict, generations = self.xnli_eval_dict[lang_s].evaluate(gen_len=5, print_logs='True', lang_s=lang_s)
                glue_results['xnli_{}'.format(lang_s)] = result_dict
                self._save_generations(record_path, generations, 'xnli_{}'.format(lang_s))
