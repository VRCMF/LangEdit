...
CONTEXT_TEMPLATES_CACHE_zh = None
...
def apply_AlphaEdit_to_model(
    ...
    for request_idx, request in enumerate(requests):
        if lang_s == 'mt':
            position_in_cycle = request_idx % len(language_set)
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        # recalculate the P 
        # ipdb.set_trace()
        if lang_null_space == 'Y':
            # if cnt not in cov_list.keys():
            #     cov_list[cnt] = []
        ...
        # upd_matrix = torch.linalg.solve(
        #         P[i,:,:].cuda() @ (layer_ks @ layer_ks.T) + 2*hparams.L2*torch.eye(layer_ks.shape[0], dtype=torch.float,device="cuda"), P[i,:,:].cuda() @ layer_ks @ resid.T
        # )
...
def get_project(model, tok, layer, hparams, cov, new_cov, cnt):
    ...
    if new_cov != None:
        ...
        # cov
        # for idx in range()
        # cov = ((100000 + (cnt-1)*100)/(100000 + cnt*100))*cov + (100/(100000 + cnt*100))*new_cov
...
def get_context_templates(model, tok, lang_s): 
    global CONTEXT_TEMPLATES_CACHE_en
    global CONTEXT_TEMPLATES_CACHE_fr
    global CONTEXT_TEMPLATES_CACHE_zh
    ...
    # ============== nl ==========
        ...
        if CONTEXT_TEMPLATES_CACHE_zh is None:
            CONTEXT_TEMPLATES_CACHE_zh = [["{}"]] + [
                [
                    f.replace("{", " ").replace("}", " ") + ". {}"
                    for f in generate_fast(
                        model,
            ...
            print(f"Cached context templates {CONTEXT_TEMPLATES_CACHE_zh}")
        return CONTEXT_TEMPLATES_CACHE_zh
